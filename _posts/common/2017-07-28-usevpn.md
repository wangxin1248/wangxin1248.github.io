...
![01](/static/assets/img/blog/2017-06/01.jpg)
...
![vpnsetup]((/static/assets/img/blog/2017-06/vpnsetup.png)

安装之后运行它：

![vpnnew]((/static/assets/img/blog/2017-06/vpnnew.png)

在这里点**新建**：

![vpnset]((/static/assets/img/blog/2017-06/vpnset.png)

Host Name填服务器的地址或域名，端口如果之前改过了在这也记得改过来，右下角的密码填之前设置过的密码。新建完成后Connect就会弹出Easy Setup窗口（如果提示连不上请检查一下服务器的防火墙设置）：

![vpneasy]((/static/assets/img/blog/2017-06/vpneasy.png)
...
![vpnl2tp](/static/assets/img/blog/2017-06/vpnl2tp.png)
...
![vpnuser](/static/assets/img/blog/2017-06/vpnuser.png)

其中用户名是必填的，验证类型选密码验证就行，然后在右侧设置用户密码

之后回到管理界面，点管理虚拟HUB：

![vpnmanager](/static/assets/img/blog/2017-06/vpnmanager.png)

![vpnhub](/static/assets/img/blog/2017-06/vpnhub.png)

这里也可以继续添加新用户。点**虚拟NAT和虚拟DHCP服务器**，弹出窗口：

![vpnvirtual]/static/assets/img/blog/2017-06/vpnvirtual.png)

在里面启用**SecureNAT**并点**SecureNAT**配置：

![vpnnat]/static/assets/img/blog/2017-06/vpnnat.png)

注意DNS要改为**8.8.8.8**和**8.8.4.4**。

这里就算配置完毕。然后顺便生成一下**OpenVPN**的配置文件，点**OpenVPN / MS-SSTP Setting**：

![vpnopen](/static/assets/img/blog/2017-06/vpnopen.png)

然后点击新窗口中部的按钮生成配置文件：

![vpnovpn](/static/assets/img/blog/2017-06/vpnovpn.png)
...
![intent](/static/assets/img/blog/2017-06/intent.PNG)

选择连接到工作区 。

![vpn](/static/assets/img/blog/2017-06/vpn.PNG)

连接VPN

![host](/static/assets/img/blog/2017-06/host.PNG)

然后在地址栏里填上服务器地址或域名（**这里填写对应VPS的IPV6地址**）。配置完成后打开**更改适配器设置**：

![info](/static/assets/img/blog/2017-06/info.PNG)

打开VPN的属性

![security](/static/assets/img/blog/2017-06/security.PNG)

在**安全**选项卡里将**VPN类型**改为**L2TP**，然后点高级设置：

![key](/static/assets/img/blog/2017-06/key.PNG)

在里面选上使用预共享密钥并把之前设置的密钥填进去。

![vpnconnect](/static/assets/img/blog/2017-06/vpnconnect.PNG)
